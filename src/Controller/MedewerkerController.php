...
class MedewerkerController extends Controller
{
    ...
    /**
     * @Route("/admin/deelnemer/remove/{activiteit}/{deelnemer}", name="removeDeelnemer")
     */
    public function removeParticipant($activiteit, $deelnemer) {
        ...
        $user = $this->getDoctrine()
            ->getRepository('App:User')
            ->find($deelnemer);
        $user->removeActiviteit($act);
        $em = $this->getDoctrine()->getManager();
        $em->persist($user);
        $em->flush();

        return $this->redirectToRoute('details', ['id' => $activiteit]);
    }
}
